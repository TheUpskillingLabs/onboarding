...
# 1. sanity: local files vs what dev has applied (expect remote matching its migration history (dev was at 00053 when these were cut))
supabase link --project-ref <DEV_PROJECT_REF>
supabase migration list

# 2. pre-checks the doc flags
#    - existing enrollment statuses must be covered by 00056's CHECK:
...
# 4. verify
supabase migration list        # remote now shows through 00058
```

**Hold 00058 if legal hasn't signed off** on its two flagged decisions (commons-content
detach-vs-delete, auth.users deletion) — just don't copy that file yet; 00054–00057 are
additive and safe alone. Note: the new tables have RLS enabled with policies arriving in
00058, so until it lands only service-role access touches them (fine — nothing reads them
yet).
